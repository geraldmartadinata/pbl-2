        ...
        {success ? (
          <Card className="p-11 text-center animate-fade-in">
            ...
            <div className="flex gap-3 justify-center">
              <Button variant="secondary" onClick={() => navigate('/')}>Back to Events</Button>
          ...
          <>
            <Card className="p-6 mb-6">
              ...
              {event.price > 0 && <p className="text-sm text-zinc-400 mt-2">Fee: Rp {event.price.toLocaleString('id-ID')}</p>}
            </Card>

            {isFull ? (
              <Card className="p-8 text-center">
                <p className="text-red-300">This event is full.</p>
              </Card>
            ) : (
              <Card className="p-7">
                <h2 className="text-lg font-semibold text-white mb-6">Registration Form</h2>
                <form onSubmit={handleSubmit} className="space-y-4">
                  <Input label="Full Name" name="full_name" placeholder="Your full name" value={form.full_name} onChange={handleChange} error={errors.full_name} />
                  <div className="grid sm:grid-cols-2 gap-4">
                    <Input label="NIM" name="nim" placeholder="2502000001" value={form.nim} onChange={handleChange} error={errors.nim} />
                  ...
                  <Input label="Line ID" name="line_id" placeholder="Your Line ID" value={form.line_id} onChange={handleChange} error={errors.line_id} />
                  <div className="pt-4 border-t border-white/[6%]">
                    <Button type="submit" className="w-full sm:w-auto" loading={submitting}>Register Now</Button>
                  </div>
                </form>
              </Card>
            )}
